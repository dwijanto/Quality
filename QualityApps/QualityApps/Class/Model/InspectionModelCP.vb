...
Public Class InspectionModelCP
    ...
    Public Sub New()
        ...
        sb.Append("with f as (select distinct * from (select material,first_value(ccetd) over (partition by material order by ccetd asc) as ccetd from quality.dailycptx dt" &
                  ...
                  " left join quality.dailycptx dt on dt.material = f.material and dt.ccetd = f.ccetd;")
        ...
        _sqlstr = String.Format("select * from quality.inspectionreportwrkcp01({0},'{1}');", datespan, SBExecption.ToString.Replace("'", "''"))

    End Sub
    Public ReadOnly Property TableName As String
        Get
            Return "quality.dailycptx"
        End Get
    End Property
    ...
    Public Function GetRemarkHistory(ByVal po As Long, ByVal item As Integer, ByVal seqn As Integer, ByVal qty As Decimal) As DataSet
        ...
        Using conn As Object = myAdapter.getConnection
            ...
            Dim sqlstr = String.Format("select * from {0} tx" &
                                       ...
                                       "with selected as (select h.id,h.purchdoc,h.item,h.qty,h.inspdate,h.remark,h.docdate from quality.dailycptx tx" &
                                       " inner join quality.historycptx h on h.purchdoc = tx.purchdoc and h.item = tx.item and h.seqn = tx.seqn" &
                                       " where not (h.remark isnull and h.inspdate isnull))," &
                                       " alldata as (select id,purchdoc,item,qty,inspdate,remark,docdate from quality.historycptx h where not (remark isnull and inspdate isnull))," &
                                       " missingdata as (select * from alldata except all select * from selected) " &
                                       " select * from missingdata where purchdoc =:po and item = :item;", "quality.historycptx")
    ...
    Public Function UpdateSeqNHistory(ByVal id As Long, ByVal seqn As Integer) As Boolean
        ...
        Dim sqlstr = "update quality.historycptx set seqn=:seqn where id =:id;"
    ...
    Function LoadDataAllMissing(DS As DataSet) As Boolean
        ...
        Using conn As Object = myAdapter.getConnection
            conn.Open()
            sqlstr = String.Format("with  selected as (select h.id,h.purchdoc::text,h.item,h.qty,h.remark,h.docdate from quality.dailycptx tx" &
                                   " inner join quality.historycptx h on h.purchdoc = tx.purchdoc and h.item = tx.item and h.seqn = tx.seqn" &
                                   " where not h.remark isnull)," &
                                   " alldata as (select id,purchdoc::text,item,qty,remark,docdate from quality.historycptx h where not remark isnull and purchdoc in (select purchdoc from quality.dailycptx))," &
                                   " missingdata as (select * from alldata except all select * from selected) " &
    ...
    Function LoadDataAllMissingInspDate(DS As DataSet) As Boolean
        ...
        Using conn As Object = myAdapter.getConnection
            conn.Open()
            sqlstr = String.Format("with  selected as (select h.id,h.purchdoc::text,h.item,h.qty,h.inspdate,h.docdate from quality.dailycptx tx" &
                                   " inner join quality.historycptx h on h.purchdoc = tx.purchdoc and h.item = tx.item and h.seqn = tx.seqn" &
                                   " where not h.inspdate isnull)," &
                                   " alldata as (select id,purchdoc::text,item,qty,inspdate,docdate from quality.historycptx h where not inspdate isnull and purchdoc in (select purchdoc from quality.dailycptx))," &
                                   " missingdata as (select * from alldata except all select * from selected) " &
                                   " select * from missingdata order by docdate desc,purchdoc")
    ...
    Public Function savehistory(ByVal obj As Object, ByVal mye As ContentBaseEventArgs) As Boolean
        ...
        Using conn As Object = myAdapter.getConnection
            ...
            'Update
            Dim sqlstr = "quality.sp_update_historycptx"
            ...
            sqlstr = "quality.sp_insert_historycptx"
            ...
            sqlstr = "quality.sp_delete_historycptx"
    ...
    Public Function GetLatestHistoryRecord() As Date
        ...
        myresult = myAdapter.RunStoreProcedure("quality.getinspdatecp", dbparams)
        Return myresult
    End Function
    ...
    Public Function GetLatestRemarksHistoryRecord() As String
        ...
        myresult = myAdapter.RunStoreProcedure("quality.getlatestremarkcp", dbparams)
        Return myresult
    End Function
End Class
