...
Public Class VendorAssignmentModel
    ...
    Public Function LoadData(DS As DataSet) As Boolean Implements IModel.LoadData
        ...
        Using conn As NpgsqlConnection = myadapter.getConnection
            conn.Open()
            sb.Append(String.Format("select u.vendorcode::text,v.vendorname::text,v.shortname::text,qu.username,u.fgcp,u.id,quality.getqename(u.fgcp)as qe,u.userid from {0} u left join vendor v on v.vendorcode = u.vendorcode" &
                                   " left join quality.user qu on lower(qu.userid) = lower(u.userid)  order by {1};", tablename, sortField))
            sb.Append(String.Format("select *,v.vendorcode::text || ' - ' || v.shortname::text || ' - ' || v.vendorname as description from vendor v order by v.shortname;"))
