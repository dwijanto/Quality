    ...
    Public Function LoadData(DS As DataSet) As Boolean Implements IModel.LoadData
        ...
        Using conn As NpgsqlConnection = myadapter.getConnection
            ...
            sb.Append(String.Format("with st as (select pd.ivalue as id,pd.cvalue as statusname from quality.paramdt pd" &
                                    ...
                                    " bu as (select pd.ivalue as id,pd.cvalue as buname from quality.paramdt pd " &
                                    " left join quality.paramhd ph on ph.paramhdid = pd.paramhdid where ph.paramname = 'BU') select u.*,st.statusname,bu.buname,c.materialdesc,v.vendorname,c.cmmf || '-' || c.materialdesc as cmmfdescription,v.vendorcode || '-' || v.vendorname as vendordescription from {0} u " &
                                    " left join cmmf c on c.cmmf = u.cmmf left join vendor v on v.vendorcode = u.vendorcode left join st on st.id = u.status left join bu on bu.id = u.sbuid order by {1};;", tablename, sortField))
            ...
            sb.Append("select pd.ivalue as id,pd.cvalue as statusname from quality.paramdt pd" &
                      " left join quality.paramhd ph on ph.paramhdid = pd.paramhdid" &
                      " where ph.paramname = 'HighRiskStatus';")
            sb.Append("select pd.ivalue as id,pd.cvalue as buname from quality.paramdt pd" &
                     " left join quality.paramhd ph on ph.paramhdid = pd.paramhdid" &
                     " where ph.paramname = 'BU';")
            sqlstr = sb.ToString
