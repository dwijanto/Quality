...
Public Class ImportDailyTxCP
    Inherits BaseImport
    Implements IImport
    Private MyForm As FormImportDailyExtractionCP
    Private TXSB As StringBuilder
    Private myAdapter As PostgreSQLDBAdapter = PostgreSQLDBAdapter.getInstance

    Public Sub New(ByVal parent As FormImportDailyExtractionCP)
        Me.MyForm = parent
    End Sub
    ...
    Public Function CopyTx() As Boolean Implements IImport.CopyTx
        ...
        If TXSB.Length > 0 Then
            ...
            If TXSB.Length > 0 Then
                ...
                '    Return False
                'End If
                Sqlstr = "delete from quality.dailycptx;select setval('quality.dailycptx_id_seq',1,false);begin;set statement_timeout to 0;end;copy quality.dailycptx(porg,cocd,plant,cc,purchdoc,item,seqn,insplot,stage,inspector,code,vendor,vendorname,material,ntsg,materialdesc," &
