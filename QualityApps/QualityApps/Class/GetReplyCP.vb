...
Imports System.Text
Imports System.IO
Public Class GetReplyCP
    Inherits BaseImport
    Dim myParam As ParamAdapter = ParamAdapter.getInstance
    Dim INBOXFolder As String = myParam.GetMailFolder("INBOXCP")
            ...
            For Each t In myTexTFile
                ...
                Using objTFParser = New FileIO.TextFieldParser(t)
                    ...
                    If myList.Count > 0 Then
                        ...
                        deleteSB.Append(String.Format("delete from quality.historycptx where docdate = '{0}' and vendor = {1};", buff(0), buff(1)))
                    End If
    ...
    Public Function CopyTx() As Boolean
        ...
        If Recordsb.Length > 0 Then
            ...
            If Recordsb.Length > 0 Then
                ...
                Sqlstr = deleteSB.ToString & "begin;set statement_timeout to 0;end;copy quality.historycptx( purchdoc,item , seqn ,vendor,ccetd, qty , inspdate ,remark, docdate,productionenddate)  from stdin with null as 'Null';"
